...
@router.post("/dolgs/payment", response_model=NeedToPay)
async def handler(data: GroupDolgRequest, current_user: User = Depends(get_current_user)):
    bills = await get_group_bills(group_id=data.id)
    res = await create_payment(data = PaymentUrlRequest([NeedToPayRequest(bill_id=i.id) for i in bills]), current_user=current_user)
